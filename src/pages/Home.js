...
const Home = () => {
    ...
    useEffect(() => {
        if (isStarted) {
            setTimeout(() => {
                setMessageBottom(<TextAppear text="Somewhere, lost in space" index={1} duration={12} class="message-bottom" />);
                setInMove(false);
            }, 1000);
        }
    }, [isStarted]);
    ...
    return (
        <main className="Home">
            <ExtraspeedEffect stop={start} arrived={arrivedOnPage} page={page} direction={direction} />
            <p className={`${isStarted ? '' : 'active'}`}>press any key</p>
            <span
            ...
            <div className={`arrow-container ${inMove ? 'in-move' : ''}`}>
                <span className="material-icons" onClick={leftPage}>
                    double_arrow
                </span>
                <span className="material-icons" onClick={rightPage}>
                    double_arrow
                </span>
            </div>
            <canvas className="spaceship" />
            <BlackBottom />
        </main>
    );
};
